...
const PORT=8140,DBG=9350;
const server=spawn("python3",["-m","http.server",String(PORT)],{cwd:process.cwd(),stdio:"ignore"});await new Promise(r=>setTimeout(r,700));
const chrome=spawn(SHELL,["--headless",`--remote-debugging-port=${DBG}`,"--disable-gpu","--no-sandbox","--hide-scrollbars","--user-data-dir=/tmp/tl-tx","about:blank"],{stdio:"ignore"});await new Promise(r=>setTimeout(r,1200));
...
await send("Emulation.setDeviceMetricsOverride",{width:430,height:340,deviceScaleFactor:3,mobile:false});
...
for (const N of [10,12]) {
  await ev(`document.querySelectorAll('#matrixGrid .col-label').forEach(l=>{l.style.transform='translateX(${N}px) rotate(-52deg)';});`);
  await ev(`(function(){document.querySelectorAll('.__g').forEach(e=>e.remove());const cells=[...document.querySelectorAll('#matrixGrid .grid-row')[0].querySelectorAll('.cell')];for(const k of [0,1,2,3]){const c=cells[k];if(!c)continue;const r=c.getBoundingClientRect();const g=document.createElement('div');g.className='__g';g.style.cssText='position:fixed;top:0;height:235px;width:1px;background:rgba(220,0,0,.65);z-index:9999;left:'+(r.left+r.width/2)+'px';document.body.appendChild(g);}})()`);
  await ev(`window.scrollTo(0, ${gy} - 12)`); await wait(120);
  let r=await send("Page.captureScreenshot",{format:"png"});
  writeFileSync(`/tmp/tx_${N}.png`,Buffer.from(r.result.data,"base64"));
}
