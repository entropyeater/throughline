...
// ml-kmeans is pulled from jsDelivr (same CDN as d3) rather than Skypack, which
// proved unreliable in production (intermittent 404s → the whole module graph
// fails to load and the app renders blank). Keep both libraries on one CDN.
...
function renderMatrixList() {
  ...
  if (items.length === 0) {
    ...
    empty.textContent =
      'No matrices yet. Click "+ New matrix" to start with the Karli broad-careers list.';
...
// Signal a successful boot. The inline watchdog in index.html shows a visible
// error banner if this never runs (e.g., a failed CDN/module import), so a load
// failure is never a silent blank page.
window.__insightMatrixBooted = true;
